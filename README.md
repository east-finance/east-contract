...
- USDapTokenId - пользовательский токен, аналог USDap в блокчейне WE. Decimals = 8
...
#### mint
<b> Описание: </b>
Создаёт Vault пользователя, переводит перечисленные WEST токены из трансфера в USDP и часть оставляет для обспечения, добавляет на баланс пользователя EAST токенов. Перевод осуществляется по курсу из данных ораклов. Если vault для пользователя уже существует то вызов отменяется.  
<b> Как вызвать: </b>
